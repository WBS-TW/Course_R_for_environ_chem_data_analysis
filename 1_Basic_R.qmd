...
-   Used for parameters/information to functions, e.g., c(1,2,3) 

**\[ \]** brackets

-   Mostly used for indexing, i.e., getting location of an element, e.g. Mat\[1,3\] is value at row 1 and col 3

**{ }** curly brackets

-   Used for encompassing body (expressions) in a function

**/** forward slash

-   R uses forward slash `/` when indicating file path, which is different from Windows that uses backslash `\`. If you get an error when inputting a file path, then check if you have the correct forward slash (or you can also use double backslash `\\`).
...
What happens when we perform operations with two or more vectors of different length? First, we can use the `length()` function to check the length of a vector:
...
Example 2. The object `letters` is a built constant in to base R and can be called to generate a sequence of lower case alphabetic letters. Yo can index it to subset the different letters.

```{r}

letters

matrix(data = letters[1:10], nrow = 5)

```

You dont need to specify both ncol or nrow, then R will automatically fit based on the one specified nrwo or ncol argument.
...
To subset an element within the list you can use index [@rspatial]. There is difference between single `[]` and double brackets`[[]]`. Can you see the differences between the below indexing?
